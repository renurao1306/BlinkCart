...
export default function UnassignedOrders() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const res = await fetch("http://52.66.211.139:5000/api/delivery/orders/unassigned", {
                    headers: {
    ...
    const handleAcceptOrder = async (id: string) => {
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://52.66.211.139:5000/api/delivery/orders/accept/${id}`, {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${token}`,
